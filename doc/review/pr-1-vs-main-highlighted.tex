% Self-contained review snapshot for PR #1 against main (83a0305).
% Rebuild with: ./doc/review/build.sh
% Blue marks PR additions, red marks text removed from main, and pink marks
% revisions made from PDF review comments.
...
\definecolor{reviewpink}{HTML}{C21875}
\newcommand{\REV}[1]{{\color{reviewpink}#1}}
...
{\small\textcolor{blue}{Blue: PR additions}\quad
 \textcolor{red}{Red: text removed from main}\quad
 \textcolor{reviewpink}{Pink: revisions from review comments}\par}
...
\(A\cup W\). \DIFdelbegin \DIFdel{By contrast, \emph{frame-preserving reuse} also retains }\DIFdelend \DIFaddbegin \DIFadd{\REV{Under the additional reductivity assumption in Section~7,}
\emph{frame-preserving reuse} \REV{retains} }\DIFaddend the consumer's information about fields outside the
write set \(W\).
...
fields to preserve a relation represented in the abstract state. The operation \(\filter^\sharp\)
\DIFdelbegin \DIFdel{retains information about }\DIFdelend \DIFaddbegin \DIFadd{returns an abstraction that is sound for }\DIFaddend the selected fields\DIFaddbegin \DIFadd{; when filtering is exact, it retains
exactly that information }\DIFaddend and discards information about all other fields. Their types and the
semantic properties they must satisfy are specified below. \DIFaddbegin \DIFadd{Every state component that can affect
the semantics must either be represented by \(\Field\) or held fixed by the statement and cache
key, \REV{as required by the expansion and frame assumptions below.}
}\DIFaddend
...
needed to preserve relations including the fields already selected. \DIFdelbegin \DIFdel{Together, they ensure that
retaining the fields in \(A\) is enough to reproduce the possible outputs of }\DIFdelend \DIFaddbegin \DIFadd{The conditions have distinct
roles: \(R\subseteq A\), together with Read/Write Expansion, establishes semantic sufficiency for
executing }\DIFaddend \(s\)\DIFdelbegin \DIFdel{on \(A\cup W\), and let us obtain an exact
projection of the relational }\DIFdelend \DIFaddbegin \DIFadd{; closure under \(\relate^\sharp\), together with Closed Exact, establishes exact
filtering of the }\DIFaddend producer state.

To construct a smaller replay set when \DIFdelbegin \DIFdel{only finitely many fields can be added}\DIFdelend \DIFaddbegin \DIFadd{the reachable field universe has finite height}\DIFaddend , start from
the read set and repeatedly add relational dependencies:
\[
  A_0=R,
  \qquad
...
\REV{We assume that the read and write sets satisfy the following semantic property:}
\[
  R\subseteq A
...
\DIFdelbegin \DIFdel{Production analyzes \(I^\sharp\) before filtering it. Therefore,
...
}\DIFdelend \DIFaddbegin \DIFadd{\REV{The stored-input condition needed for Summary Sound is the following upper bound:}
}\[
  \DIFadd{\gamma(I_c^\sharp)
  \subseteq
  \ProjClose\!\left(A,\gamma(I^\sharp)\right).
  \tag{Production Input}
}\]
\DIFadd{\REV{Exact producer filtering satisfies Production Input by Proposition~2. A producer may instead
establish Production Input directly.}
}\DIFaddend
...
\DIFaddbegin \DIFadd{\REV{Filtered-key equality is sufficient for Match Sound. With noncanonical abstract representations,
it may reject a semantically valid match. Such rejection reduces reuse opportunities but preserves
soundness.}
}\DIFaddend
...
\[
  ...
  \tag{Write Frame}
\]
\DIFaddbegin \DIFadd{\REV{Frame preservation requires the consumer filter to be reductive:}
}\[
  \DIFadd{\gamma\!\left(\filter^\sharp(\Field\setminus W,J^\sharp)\right)
  \subseteq
  \ProjClose\!\left(\Field\setminus W,\gamma(J^\sharp)\right).
  \tag{Frame Filter Reductive}
}\]
\DIFadd{\REV{Combined with Filter Sound, Frame Filter Reductive makes this filter exact.}
}\DIFaddend The abstract domain also provides a meet operation:
\[
...
\DIFdelbegin \DIFdel{Together, these properties make the meet represent the intersection of the two concretizations
...
}\DIFdelend \DIFaddbegin \DIFadd{\REV{When lookup succeeds, frame-preserving reuse meets the stored output with the consumer input
filtered to \(\Field\setminus W\):}
}\DIFaddend \[
  \frameReuse^\sharp\DIFaddbegin \DIFadd{:\mathbb D^\sharp\rightharpoonup\mathbb D^\sharp,
  \qquad
  \frameReuse^\sharp}\DIFaddend (J^\sharp)
...
\begin{statementbox}
\DIFdelbegin \DIFdel{\boxheading{Proposition 5 --- Sound and precise frame-preserving reuse}
}\DIFdelend \DIFaddbegin \DIFadd{\boxheading{Proposition 5 --- Sound, precise, and frame-preserving reuse}
}\DIFaddend When lookup succeeds, frame-preserving reuse is sound\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend at least as precise as coarse reuse\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{, and
respects the consumer frame:
}\DIFaddend \begin{align*}
  &\text{Sound:}
  &&\SoundReuse\!\left(J^\sharp,\frameReuse^\sharp(J^\sharp)\right),\\
  &\text{Precise:}
  &&\gamma\!\left(\frameReuse^\sharp(J^\sharp)\right)
    \subseteq\gamma\!\left(\reuse^\sharp(J^\sharp)\right)\DIFaddbegin \DIFadd{,}\\
  &\DIFadd{\text{Frame:}
  }&&\DIFadd{\gamma\!}\left(\DIFadd{\frameReuse^\sharp(J^\sharp)}\right)
    \DIFadd{\subseteq\ProjClose\!}\left(\DIFadd{\Field\setminus W,\gamma(J^\sharp)}\right)\DIFaddend .
\end{align*}
\end{statementbox}
...
\[
  ...
  =\gamma\!\left(\reuse^\sharp(J^\sharp)\right).
\]
Therefore, frame-preserving reuse admits no more concrete states than coarse reuse. \DIFaddbegin \qedbox

\DIFadd{\textbf{Proof of Frame.}
\REV{Meet Reductive and Frame Filter Reductive give}
}\[
  \DIFadd{\gamma\!\left(\frameReuse^\sharp(J^\sharp)\right)
  \subseteq
  \gamma\!\left(\filter^\sharp(\Field\setminus W,J^\sharp)\right)
  \subseteq
  \ProjClose\!\left(\Field\setminus W,\gamma(J^\sharp)\right).
}\]
\DIFadd{\REV{This is the Frame conclusion.} }\DIFaddend \qedbox
