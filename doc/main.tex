...
\(A\cup W\). By contrast, \emph{frame-preserving reuse} is designed to retain the consumer's
information about fields outside the write set \(W\). Section~7 states the additional
reductivity contract that makes this guarantee semantic rather than merely operational.
...
returns an abstraction that is sound for the selected fields; when filtering is exact, it retains
exactly that information and discards information about all other fields. Their types and the
semantic properties they must satisfy are specified below. Every state component that can affect
the semantics must either be represented by \(\Field\) or held fixed by the statement and cache
key; the explicit expansion and frame properties below are the semantic certificates used by the
proofs.
...
needed to preserve relations including the fields already selected. The conditions have distinct
roles: \(R\subseteq A\), together with Read/Write Expansion, establishes semantic sufficiency for
executing \(s\); closure under \(\relate^\sharp\), together with Closed Exact, establishes exact
filtering of the producer state. Neither conclusion follows from Replay Closure alone.

To construct a smaller replay set when the reachable field universe has finite height, start from
...
The informal may-read and may-write descriptions do not by themselves prove the required
noninterference result. We therefore assume the following independent semantic property:
\[
  R\subseteq A
...
\]
The proof below needs only the instance \(P=\gamma(I^\sharp)\); the universal form is a reusable
certificate for other producer inputs.
...
closure. Separation is displayed as an independent diagnostic property: it is not a premise of
Propositions~1--5, is not implied by Replay Closure, and does not by itself establish either
representability or exact filtering. Those obligations are carried explicitly by Closed Exact.
...
Exact producer filtering is a sufficient uniform construction and requires the agreement closure
to be representable in \(\mathbb D^\sharp\). The production proof itself uses only the upper
bound
\[
  \gamma(I_c^\sharp)
  \subseteq
  ...
  \tag{Production Input}
\]
Filter Sound supplies the opposite inclusion, so the uniform filter used here satisfies both
directions exactly. A producer may instead discharge Summary Sound directly or establish
Production Input by another construction. Sound filtering remains sufficient for the stored
output and for deriving Match Sound from filtered-key equality.
...
Thus Match Sound is the semantic lookup contract; filtered abstract equality is one sufficient
implementation. Noncanonical abstract representations may cause this equality test to reject a
semantically valid match, but such false negatives do not affect soundness.
...
\[
  ...
  \tag{Write Frame}
\]
Sound filtering alone may return an arbitrarily coarse abstraction. Actual preservation of the
consumer frame therefore also requires the selected consumer filter to be reductive:
\[
...
\]
Together with Filter Sound, this makes the consumer frame filter exact for this lookup.
...
\[
  \frameReuse^\sharp:\mathbb D^\sharp\rightharpoonup\mathbb D^\sharp,
  \qquad
...
\begin{statementbox}
\boxheading{Proposition 5 --- Sound, precise, and frame-preserving reuse}
When lookup succeeds, frame-preserving reuse is sound, at least as precise as coarse reuse, and
...
\begin{align*}
  ...
  &\text{Frame:}
  &&\gamma\!\left(\frameReuse^\sharp(J^\sharp)\right)
    \subseteq\ProjClose\!\left(\Field\setminus W,\gamma(J^\sharp)\right).
\end{align*}
\end{statementbox}
...
\textbf{Proof of Frame.}
Meet Reductive gives
\[
  \gamma\!\left(\frameReuse^\sharp(J^\sharp)\right)
  \subseteq
  \gamma\!\left(\filter^\sharp(\Field\setminus W,J^\sharp)\right).
\]
Frame Filter Reductive then gives
\[
  \gamma\!\left(\frameReuse^\sharp(J^\sharp)\right)
  \subseteq
  \ProjClose\!\left(\Field\setminus W,\gamma(J^\sharp)\right).
\]
Thus the result retains the consumer information outside the write set. \qedbox
